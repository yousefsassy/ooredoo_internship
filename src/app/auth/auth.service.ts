...
export class AuthenticationService {
  ...
  private baseUrl = 'http://localhost:8089/Ooredoo/auth';
...
register(userData: any): Observable<any> {
  ...
  resetPassword(token: string, newPassword: string): Observable<void> {
    return this.http.post<void>(`${this.baseUrl}/ResetPassword`, { token, newPassword }).pipe(
...
registerWithGoogle(username: string, email: string): Observable<any> {
  ...
  return this.http.get<Shop>(`http://localhost:8089/Ooredoo/shops/${id}`);
}

updateShop(shop: Shop): Observable<any> {
  return this.http.put(`http://localhost:8089/Ooredoo/shops/${shop.idShop}`, shop);
}

archiveShop(id: number): Observable<any> {
  return this.http.put(`http://localhost:8089/Ooredoo/shops/${id}/archive`, {});
}

deleteShop(id: number): Observable<any> {
  return this.http.delete(`http://localhost:8089/Ooredoo/shops/${id}`);
}
